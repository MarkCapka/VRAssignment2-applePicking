...
public class HapticColorChangeGrabber : MonoBehaviour 
{
	public int grabberButtonID = 0;     //!< index of the button assigned to grabbing.  Defaults to the first button
	public int colorChangeButtonID = 1;
	...
	private bool grabberButtonStatus = false;           //!< Is the button currently pressed?
	private bool colorChangeButtonStatus = false;
	...
	void FixedUpdate () 
	{
		bool newGrabberButtonStatus = hapticDevice.GetComponent<HapticPlugin>().Buttons [grabberButtonID] == 1;
		bool oldGrabberButtonStatus = grabberButtonStatus;
		grabberButtonStatus = newGrabberButtonStatus;

		bool newColorChangeButtonStatus = hapticDevice.GetComponent<HapticPlugin>().Buttons[colorChangeButtonID] == 1;
		bool oldColorChangeButtonStatus = colorChangeButtonStatus;
		colorChangeButtonStatus = newColorChangeButtonStatus;

		if (oldGrabberButtonStatus == false && newGrabberButtonStatus == true)
		{
			Debug.Log("Button 0 changed status.");
			Debug.Log("Buttons: " + string.Join(" ", hapticDevice.GetComponent<HapticPlugin>().Buttons));
		...
		if (oldGrabberButtonStatus == true && newGrabberButtonStatus == false)
		{
			if (ButtonActsAsToggle)
			{
				//Do Nothing
		...
		/*
		if (oldColorChangeButtonStatus == false && newColorChangeButtonStatus == true)
		{
			Debug.Log("Changing object color by touch.");
			changeTouchedColor();
		}
		*/
	...
	void changeTouchedColor()
    {
		...
		Color randomlySelectedColor = new Color(
		   Random.Range(0f, 1f),
		   Random.Range(0f, 1f),
		   Random.Range(0f, 1f),
		   0.25f);
				touchedObject.GetComponent<Renderer>().material.color = randomlySelectedColor;
	}
